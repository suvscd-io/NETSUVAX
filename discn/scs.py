        ...
        for target in targets_str.split(','):
            ...
            try:
                if '/' in target:
                    ...
                    # Handle IP ranges like 192.168.1.1-192.168.1.10
                ...
                console.print(f"[yellow]Warning: Invalid target '{target}': {e}[/yellow]")
                
        if not targets:
            raise ValueError("No valid targets found after parsing")
        return sorted(list(targets), key=ipaddress.ip_address)
            ...
            for port_spec in ports_str.split(','):
                ...
                if '-' in port_spec:
                    ...
                    try:
                        ...
                        for port in range(start_port, min(end_port + 1, start_port + 10000)):
                            ports.add(port)
                        
                        if end_port - start_port > 10000:
                            console.print(f"[yellow]Warning: Limiting port range {port_spec} to first 10000 ports[/yellow]")
                    except ValueError as e:
    ...
    def _identify_service(self, port, banner):
        ...
        if banner:
            ...
            if "ssh" in banner_lower:
                ...
                service = "HTTP" + ("S" if "ssl" in banner_lower or port == 443 else "")
